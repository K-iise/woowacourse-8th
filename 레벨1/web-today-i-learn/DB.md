...
1. `attendance` 테이블은 중복된 데이터가 쌓이는 구조이다. 중복된 데이터는 어떤 컬럼인가?
   - **crew_id** 와 **nickname**이 중복된 컬럼이라고 생각한다. 데이터를 삽입할 때 마다 출석할 때마다 **crew_id**, **nickname**이 중복되게 삽입되고 있다.
2. `attendance` 테이블에서 중복을 제거하기 위해 `crew` 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?
   - **crew_id** 와 **nickname**을 컬럼으로 가지고 Primary Key를 **crew_id**로 한다.
...
1. `crew` 테이블을 만들고 중복을 제거했다. `attendance`에서 불필요해지는 컬럼은?
   - `attendance` 테이블의 `nickname` 컬럼이 불 필요해진다. 

2. 컬럼을 삭제하려면 어떻게 해야 하는가?
```sql
alter table attendance drop nickname;
```
...
만약에 `crew` 테이블에는 crew_id가 12번인 크루가 존재하지 않지만, `attendance` 테이블에는 여전히 `crew_id`가 `12`번인 크루가 존재한다면?
- 해당 크루가 중간에 퇴소했거나
- 누군가의 실수에 의해 레코드가 삭제되었거나

`attendance` 테이블에서 `crew_id` 컬럼을 `crew`테이블의 기본키에 대한 외래키로 설정한다.
```sql
...
- `crew`테이블에서 `nickname`컬럼에 `unique` 제약 조건을 설정한다.
...
> 어셔: 안녕하세요 검프. 저는 3월 6일 09시 31분에 등교하고 18시 01분에 하교했습니다. 감사합니다. <br>
> 검프: 네 ^^;;; (이거 어쩌나...)
...
FROM `crew` AS c
JOIN `attendance` AS a ON c.crew_id = a.crew_id
WHERE a.attendance_date = '2026-03-06';
```
...
values ((select crew_id from crew where nickname = '어셔'), '2026-03-06', '20:00', '21:00');
```
...
select * 
from `crew` as a
join `attendance`as b on a.crew_id = b.crew_id;
```
...
```sql
select crew_id, count(*) as record_day
from attendance
group by crew_id;
``` 
...
```sql
select `attendance_date`, count(*) as record_crew
from `attendance`
